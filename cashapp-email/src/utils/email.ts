 ...
 */
export async function convertToEmail(stringMessage: string): Promise<Email> {
    try {
...
export function attachType(email: Email): string {
    try {
        const iemail: IEmail = email
        if (email.from.address == 'cash@square.com')
            iemail.type = EmailTypes.CASH_APP


        return JSON.stringify(iemail) // Convert the parsed email object to a JSON string
    } catch (error) {
        console.error("Error converting email to JSON:", error) // Log any errors encountered
        throw new Error("Failed to convert email to JSON") // Throw a new error if conversion fails
    }
