...
export function determinHookUrl(email: Email, env: any): string {
    if (email.from.address == 'cash@square.com')
        return env.CASHAPP_WEBHOOK_URL
    if (email.from.address == 'ashdowning@gmail.com')
        return env.DEBUG_WEBHOOK_URL

    return env.OTHER_WEBHOOK_URL
}
