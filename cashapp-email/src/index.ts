...
import { attachType, convertToEmail, convertToEML } from './utils/email';
import { determinHookUrl, sendWebhook } from './utils/webhook';
import { ForwardableEmailMessage } from '@cloudflare/workers-types';

// Configure your webhook URL and a shared secret
const WEBHOOK_URL = 'http://localhost:6589/webhooks/cashapp'; // Replace with the actual URL of your webhook endpoint
...
export default {
	 ...
	 */
	//@ts-ignore
	async email(message: ForwardableEmailMessage, env: any, ctx?: any): Promise<ExportedHandler<Env>> {
		const email = message;
		...
		const senderName = subject?.split('sent you')[0];
		...
		try {
			...
			const rawEmail = await new Response(email.raw).text();
		...
		// console.log('Content to emailText:', emailText);
		// console.log('Content to emailHtml:', emailHtml);
		if (contentToParse) {
		...
		const cashtag = contentToParse.split('Payment from ')[1].split('\n')[0];
		transactionId = '#' + contentToParse.split('#')[1].split('\n')[0];
		console.log(`Extracted - Amount: ${amount}, Sender: ${senderName}, Transaction ID: |${transactionId}| and Cashtag: |${cashtag}|`);
