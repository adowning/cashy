...
async function handleRequest(request: Request): Promise<Response> {
	...
	try {
		...
		if (config.allowedDomains.length > 0) {
			const isAllowed = config.allowedDomains.some((domain) => targetURL?.hostname === domain || targetURL?.hostname.endsWith(`.${domain}`));
			if (!isAllowed) {
	...
	const isWikipediaSite =
		config.specialSites.wikipedia.enabled && config.specialSites.wikipedia.domains.some((domain) => targetURL?.hostname.endsWith(domain));
	...
	headersToKeep.forEach((header) => {
		if (request.headers.has(header)) {
			newHeaders.set(header, request!.headers!.get(header) as string);
		}
	});
	...
	const isXHR = request.headers.get('X-Requested-With') === 'XMLHttpRequest' || (request.headers.get('Accept')?.includes('application/json') || false);
	...
	// Create new request
	let newRequest = new Request(targetURL!.href, {
	...
	try {
		...
		if (setCookieHeaders) {
			...
			setCookieHeaders.forEach((cookie: any) => {
...
/******************** 0dbeda61-8e40-4732-a38d-8dfdf81e7873 ********************/
...
class LinkRewriter {
	...
	proxyDomain: any;
	constructor(baseURL: any, attributeName: string, proxyDomain: any) {
		this.baseURL = baseURL;
		this.attributeName = attributeName;
		this.proxyDomain = proxyDomain;
	}

	element(element: { getAttribute: (arg0: any) => any; tagName: string; setAttribute: (arg0: string, arg1: string) => void; }) {
...
class SrcsetRewriter {
	baseURL: any;
	proxyDomain: any;
	constructor(baseURL: any, proxyDomain: any) {
		this.baseURL = baseURL;
		this.proxyDomain = proxyDomain;
	}

	element(element: { getAttribute: (arg0: string) => any; setAttribute: (arg0: string, arg1: any) => void; }) {
		...
		try {
			...
			const newSrcsetParts = srcsetParts.map((part: { trim: () => { (): any; new(): any; split: { (arg0: RegExp): [any, any]; new(): any; }; }; }) => {
...
class StyleElementRewriter {
	...
	element(element: { onEndTag: (arg0: (endTag: any) => void) => void; replace: (arg0: any) => void; }) {
		// We need to rewrite all URLs in the style element
		element.onEndTag((endTag: { before: any; name: any; after: any; }) => {
			element.replace(endTag.before + endTag.name + endTag.after);
		});
	}

	text(text: { text: any; replace: (arg0: any) => void; }) {
...
function rewriteCSS(css: string, baseURL: any | URL | undefined, proxyDomain: any) {
	if (!css) return css;

	// First handle @import statements
	css = css.replace(/@import\s+(?:url\(\s*['"]?([^'")]+)['"]?\s*\)|['"]([^'"]+)['"]).*/g, function (match: string, urlMatch: any, directMatch: any) {
	...
	// Handle url() patterns
	css = css.replace(/url\(\s*(['"]?)([^'")]+)(['"]?)\s*\)/g, function (match: any, quote1: any, url: string, quote2: any) {
		if (!url) return match;
	...
	css = css.replace(/image-set\(\s*(?:[^)]|(?:\([^)]*\)))+\)/g, function (match: string) {
		return match.replace(/url\(\s*(['"]?)([^'")]+)(['"]?)\s*\)/g, function (urlMatch: any, quote1: any, url: string, quote2: any) {
...
function rewriteJavaScript(js: string, baseURL: any, proxyDomain: any) {
	if (!js) return js;
	...
	return js
		.replace(/'(https?:\/\/[^']+)'/g, function (match: any, url: string) {
			if (url.startsWith(`https://${proxyDomain}/`)) return match;
			try {
...
class HeadRewriter {
	originalURL: any;
	constructor(originalURL: any) {
		this.originalURL = originalURL;
	}

	element(element: { append: (arg0: string, arg1: { html: boolean; }) => void; }) {
		element.append(
			`
      <script>
