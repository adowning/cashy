...
const pendingTransactions = computed(() =>
  currentUser.activeProfile.transactions.filter(
    (t: { status: string }) => t.status === 'PENDING_PAYMENT',
  ),
)
const target = ref()
const trans = authStore.userInfo.activeProfile.transactions
...
watch(authStore.userInfo.activeProfile.transactions, (newVal) => {
  console.log(newVal)
