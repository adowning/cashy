...
import { type TransactionHistoryResponse } from 'shared/interface/transaction'
import type * as Transaction from 'shared/interface/transaction'
...
export const useBonusTransactionStore = defineStore('bonusTransaction', () => {
  ...
  const dispatchTransactionHistory = async (data: any) => {
    setSuccess(false)
    const route: string = NETWORK_CONFIG.TRANSACTION_PAGE.TRANSACTION_HISTORY
