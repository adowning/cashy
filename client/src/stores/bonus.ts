...
import * as Bonus from 'shared/interface/bonus'
...
export const useBonusStore = defineStore('bonus', () => {
  ...
  const dispatchUserBonus = async () => {
    setSuccess(false)
    const route: string = NETWORK_CONFIG.BONUS_PAGE.USER_BONUS
  ...
  const dispatchBonusCancel = async (data: any) => {
    setSuccess(false)
    const route: string = NETWORK_CONFIG.BONUS_PAGE.BONUS_CANCEL
    const network: Network = Network.getInstance()
