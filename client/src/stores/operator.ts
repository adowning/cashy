import type { Game, Operator, Product, User } from 'shared/'
...
export const useOperatorStore = defineStore(
  ...
  () => {
    ...
    const currentShop = ref<Operator>()
    const userList = ref<Partial<User[]>>([])
    ...
    //   bigWins.value = wins
    // }
    const setShop = (shopInfo: Operator) => {
      currentShop.value = shopInfo
    }

    const setUsers = (list: Partial<User[]>) => {
      userList.value = list
    }
...
export function useOperatorStoreOutside() {
