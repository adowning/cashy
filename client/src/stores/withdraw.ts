...
import type * as Withdraw from 'shared/interface/withdraw'
...
export const useWithdrawStore = defineStore('withdraw', () => {
  ...
  const dispatchUserWithdrawCfg = async () => {
    setSuccess(false)
    const route: string = NETWORK_CONFIG.WITHDRAW_PAGE.WITHDRAWAL_CONFIG
  ...
  const dispatchUserWithdrawSubmit = async (data: Withdraw.WithdrawItem) => {
    setSuccess(false)
    const route: string = NETWORK_CONFIG.WITHDRAW_PAGE.WITHDRAWAL_SUBMIT
    const network: Network = Network.getInstance()
  ...
  const dispatchWithdrawalHistory = async (data: any) => {
    // Keeping 'any' type as in original
    setSuccess(false)
    const route: string = NETWORK_CONFIG.WITHDRAW_PAGE.WITHDRAWAL_HISTORY
    const network: Network = Network.getInstance()
  ...
  const dispatchWithdrawalRefund = async (data: any) => {
    // Keeping 'any' type as in original
    setSuccess(false)
    const route: string = NETWORK_CONFIG.WITHDRAW_PAGE.WITHDRAWAL_REFUND
    const network: Network = Network.getInstance()
