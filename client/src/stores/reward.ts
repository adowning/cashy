...
import type * as Reward from 'shared/interface/reward'
...
export const useRewardStore = defineStore('reward', () => {
  ...
  const dispatchRewardList = async () => {
    setSuccess(false)
    const route: string = NETWORK_CONFIG.Reward.REWARD_LIST
  ...
  const dispatchReceiveAchievementBonus = async () => {
    setSuccess(false)
    const route: string = NETWORK_CONFIG.Reward.RECIEVE_ACHIV_BONUS
    const network: Network = Network.getInstance()
