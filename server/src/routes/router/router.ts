 ...
 */
export class WebSocketRouter<T extends Record<string, unknown> = Record<string, never>> {
  ...
  public upgrade(req: Request, options: UpgradeOptions<WebSocketData<T>>) {
    ...
    if (!upgraded) {
      return new Response("Failed to upgrade the request to a WebSocket connection", {
        status: 500,
        headers: {
  ...
  onMessage<Schema extends MessageSchemaType>(
    ...
    if (this.handlers.message.has(messageType)) {
      console.warn(`Handler for message type "${messageType}" is being overwritten.`);
    }
    ...
    this.handlers.open.forEach((handler) => {
      try {
        ...
        if (result instanceof Promise) {
          result.catch((error) => {
            console.error(`Unhandled promise rejection in open handler for ${clientId}:`, error);
          });
        }
      } catch (error) {
  ...
  private handleClose(ws: ServerWebSocket<WebSocketData<T>>, code: number, reason?: string) {
    const clientId = ws.data.clientId;
    console.log(`[ws] Connection closed: ${clientId} (Code: ${code}, Reason: ${reason || "N/A"})`);
    ...
    this.handlers.close.forEach((handler) => {
      try {
        ...
        if (result instanceof Promise) {
          result.catch((error) => {
            console.error(`[ws] Unhandled promise rejection in close handler for ${clientId}:`, error);
          });
        }
      } catch (error) {
  ...
  private handleMessage(ws: ServerWebSocket<WebSocketData<T>>, message: string | Buffer) {
    const clientId = ws.data.clientId;
    ...
    try {
      ...
      if (
        ...
        console.warn(`[ws] Received invalid message format from ${clientId}:`, parsedMessage);
    ...
    if (!handlerEntry) {
      console.warn(`[ws] No handler found for message type "${messageType}" from ${clientId}`);
    ...
    try {
      ...
      // Catch synchronous errors in handlers
      console.error(`[ws] Error in message handler for type "${messageType}" from ${clientId}:`, error);
   ...
   */
  private createSendFunction(ws: ServerWebSocket<WebSocketData<T>>): SendFunction {
