...
import type { Client, CommandInfo, User } from "@alicarti/shared";
...
export class ClientsManager {
  ...
  joinTopic(topic: Topic, ws: ServerWebSocket<Client>) {
    ...
    console.log(
      `client ${ws.data.socketId} joining topic ${topic.name} (${topic.clientsCount})`)
    if (!topic?.join) {
      return;
    }
    ...
    // add topic to the list of topics joined by the user
